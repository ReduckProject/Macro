...
function Main()
	...
	--[千枝伏藏][千枝绽蕊]
	if buff("千枝伏藏") and life() < 0.7 then
		CastX("千枝绽蕊")
	end

	if buff("千枝绽蕊") and life() > 0.7 then
		CastX("千枝伏藏")
	end
	--银光照雪
	--if v["治疗目标血量"] < 0.7 and xdis(v["治疗目标"]) < 6 and xxface(id(), v["治疗目标"]) < 130 then
	if (v["治疗目标血量"] < 0.7 or life() < 0.4) and xdis(v["治疗目标"]) < 6 then
	xcast("银光照雪", v["治疗目标"])
	end

	if nobuff("凌然天风") and cdtime("凌然天风") > 30 then
		CastX("青川濯莲")
	end

	if map() == "云湖天池" and fight() then
		CastX("青川濯莲")
	end
