...
function Main()
    ...
    --减伤
    if fight() and life() < 0.35 and nobuff("贪魔体") then
        CastX("贪魔体")
    end
...
f["正常循环"] = function()
    ...
    --生死劫
    if v["日月齐光时间"] > 0 and v["日月齐光等级"] >= 3 then
        --悬象后齐光三
    ...
    if buff("灵·日|魂·月") and v["暗尘CD"] > 13 then
        --破魔后
        CastX("生死劫")
    end
...
function CastX(szSkill, bSelf)
    ...
    --end
    --return false
    return g_func["释放技能"](szSkill, bSelf)
end
