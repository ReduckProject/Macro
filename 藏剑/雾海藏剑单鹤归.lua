--[[ 奇穴: [淘尽][清风][岱宗][景行][造化][怜光][层云][山倾][雾锁][碧归][如风][飞来闻踪]
秘籍:
九溪  2伤害 2范围
黄龙  2伤害 2效果(剑气)
云飞  2读条 1伤害 1会心
...
f["轻剑"] = function()
	...
	if cdleft(16) >= 0.5 and face() < 80 and getopt("黄龙开关") then
...
f["重剑"] = function()
	--云飞玉皇
	if v["雾锁时间"] > casttime("云飞玉皇") or bufftime("岱宗") < 3 or bufftime("雾锁") > 1.4 then
		CastX("云飞玉皇")
	end
	...
	-- 检查鹤归孤山的CD是否就绪--
    if v["鹤归CD"] <= 0 and v["剑气"] >= 70 or (bufftime("岱宗") < 13 or buff("飞来") or (v["剑气"] >= 105 and v["岱宗时间"] < 13)) then
		CastX("鹤归孤山")
	end
	-- 释放鹤归孤山
	    -- 如果成功释放了鹤归孤山，可以在这里添加后续逻辑
	    -- 例如，如果在九皋鹤野区域内，则优先释放夕照雷峰和云飞玉皇
   if v["九皋鹤野"] then
	    if CastX("夕照雷峰") then return end
	    if CastX("云飞玉皇") then return end
	    if CastX("夕照雷峰") then return end
	    if CastX("云飞玉皇") then return end
		return
	end
	...
	if v["九皋鹤野"] then
		...
		--打峰插
	------if v["峰插CD"] <= 0 and getopt("峰插开关") and nobuff("壁归") and bufftime("闻踪") > 10 or (nobuff("归山") and v["剑气"] >= 105 and bufftime("雾锁") > 4) then
    if cdleft(16) <= 0 and getopt("峰插开关") and v["云飞CD"] <= 0.5 and nobuff("碧归") and  (bufftime("闻踪") > 10 or (nobuff("归山") and v["剑气"] >= 105 and bufftime("雾锁") > 4) )then
		CastX("峰插云景")
	end
...
f["回剑气"] = function()
	...
	--莺鸣柳
	if cdleft(16) <= 0 and v["剑气"] < 60 and dis() < 8 then
		if CastX("莺鸣柳") then
			return
		end
	end
	...
	--切轻剑回剑气
	if v["九溪CD"] < 1 and v["啸日充能时间"] < 1 and v["剑气"] < 60 and nobuff("归山") then
		f["啸日"]("切轻剑回剑气")
	end
	
	--云栖松
	if dis() < 8 and v["剑气"] < 60 and nobuff("归山") then
		CastX("云栖松")
	end
