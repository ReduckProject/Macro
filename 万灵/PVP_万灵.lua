...
function Main()
    ...
    if qixue("星烨") and fight() then
        if v["幻灵印"] == 0 then
            if not nextbeast("熊") then
                --只召鹰
                setbeast({ "熊", "鹰", "狼", "大象", "野猪", "虎" })
            end
        end

        if v["幻灵印"] == 1 then
            if not nextbeast("鹰") then
                --只召鹰
                setbeast({ "鹰", "熊", "狼", "大象", "野猪", "虎" })
            end
...
f["切换目标"] = function()
    v["20尺内敌人"] = enemy("距离<25", "视线可达", "没载具", "气血最少")
    if v["20尺内敌人"] ~= 0 then
        --没目标或不是敌对
        if not rela("敌对") then
            settar(v["20尺内敌人"])
